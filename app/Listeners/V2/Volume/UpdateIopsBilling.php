...
class UpdateIopsBilling
{
    ...
    public function handle(Updated $event)
    {
        ...
        $currentActiveMetric = BillingMetric::getActiveByKey($volume, 'volume.iops');

        if (!empty($currentActiveMetric)) {
            if ($currentActiveMetric->value == $volume->iops) {
                return;
            }
        ...
        $billingMetric->key = 'volume.iops';
        $billingMetric->value = $volume->iops;
