...
class AwaitVolumes extends Job
{
    ...
    public function handle()
    {
        ...
        if (!$state->has('volume')) {
            Log::info(get_class($this) . ' : No Volumes detected in build state, skipping', ['id' => $this->model->id]);
            return;
        }

        $this->awaitSyncableResources($state->get('volume'));
    }
}
