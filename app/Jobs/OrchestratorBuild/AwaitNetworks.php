...
class AwaitNetworks extends Job
{
    ...
    public function handle()
    {
        ...
        if (!$state->has('network')) {
            Log::info(get_class($this) . ' : No Networks detected in build state, skipping', ['id' => $this->model->id]);
            return;
        }

        $this->awaitSyncableResources($state->get('network'));
    }
}
