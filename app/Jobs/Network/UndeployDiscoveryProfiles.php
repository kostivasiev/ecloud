...
class UndeployDiscoveryProfiles extends TaskJob
{
    public function handle()
    {
        ...
        foreach ($response->results as $result) {
            $network->router->availabilityZone->nsxService()->delete(
                'policy/api/v1/infra/tier-1s/' . $network->router->id . '/segments/' . $network->id . '/segment-discovery-profile-binding-maps/' . $result->id,
            );
            $this->info('Deleted discovery profile ' . $result->id);
        }
    }
}
