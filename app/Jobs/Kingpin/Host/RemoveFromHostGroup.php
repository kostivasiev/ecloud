...
class RemoveFromHostGroup extends Job
{
    private $model;
    ...
    public function handle()
    {
        ...
        try {
            ...
            $message = 'Error while removing Host ' . $host->id . ' from Host Group.';
            Log::debug($message);
            $this->fail(new \Exception($message));
            return false;
        }
        ...
        if (!$response || $response->getStatusCode() !== 200) {
            ...
            $this->fail(new \Exception('Host ' . $host->id . ' could not be removed from Host Group.'));
            return false;
        }
