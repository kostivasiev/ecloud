...
class Attach extends Job
{
    ...
    public function handle()
    {
        ...
        if ($this->instance->volumes()->get()->count() >= config('volume.instance.limit', 15)) {
            $this->instance->setSyncFailed();
            $this->volume->setSyncFailed();
            $this->fail(new \Exception('Volume ' . $this->volume->id . ' failed to attach to instance ' . $this->instance->id . ', volume limit exceeded'));
            return;
        }
